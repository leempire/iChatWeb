...
class Tag:
    ...
    def find(self, keyword):
        ...
        # start = self.text[:direction + 1].rfind('<')
        # text = self.text[start:]
        # tag_name = re.search(r'<(.*?)[ >]', text).group(1)
        # if text.find(f'</{tag_name}>') == -1:
        #     return Tag(text[:text.find('>') + 1])
        # else:
        #     end = 0
        #     end_tag = f'</{tag_name}>'
        #     while True:
        #         index = text[end:].find(end_tag)
        #         if index == -1:
        #             return Tag(text)
        #         end += index + len(end_tag)
        #         cut = text[:end]
        #         if cut.count(f'<{tag_name}') == cut.count(end_tag):
        #             return Tag(cut)
    ...
    def __getitem__(self, item):
        value = re.findall(f'{item}="(.*?)"', self.text.replace('\'', '"'), re.S)
        if value:
            return value[0]
        else:
            return None
